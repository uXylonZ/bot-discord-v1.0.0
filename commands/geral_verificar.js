        ...
        client.once(`interactionCreate`, async interaction => {
            ...
            if (interaction.customId === `captcha`) {
                ...
                if (codigo2 === codigo1) {
                    ...
                    let role = await db.get(`cargo_verificacao_${interaction.guild.id}`)  // interaction.guild.roles.cache.get(config.cargo_verificacao)
                    let cargo = interaction.guild.roles.cache.get(role)
                    if (role === null || role === false) {
                        let embed = new Discord.EmbedBuilder()
                            .setDescription(`Ainda não tenho um cargo de verificação, peça a algum administrador para criar o painel de verificação!`)
                            .setColor('Green');
